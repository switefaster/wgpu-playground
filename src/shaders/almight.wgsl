...
fn vs_main(
    ...
    let normal_base: vec3<f32> = normalize((u_transforms.model_inv_transpose * vec4<f32>(normal, 0.0)).xyz);
    let tangent_base: vec3<f32> = normalize((u_transforms.model_inv_transpose * vec4<f32>(tangent, 0.0)).xyz);
    let bitangent_base: vec3<f32> = normalize((u_transforms.model_inv_transpose * vec4<f32>(bitangent, 0.0)).xyz);
    let tangent_matrix = transpose(mat3x3<f32>(normal_base, tangent_base, bitangent_base));
    out.world_position = tangent_matrix * model_space.xyz;
    out.view_position = tangent_matrix * u_view_proj.view_pos;
    out.light_position = tangent_matrix * u_light_view_proj.light_position;
...
fn fs_main(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    let object_normal: vec4<f32> = textureSample(t_normal, s_normal, in.tex_coord);
    ...
    let normal: vec3<f32> = normalize(object_normal.rgb * 2.0 - 1.0);
    let light_dir = normalize(in.light_position - in.world_position);
    ...
    let view_dir: vec3<f32> = normalize(in.view_position - in.world_position);
